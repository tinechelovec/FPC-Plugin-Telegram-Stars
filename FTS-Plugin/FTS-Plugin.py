...
HUMAN_LOGS=bool(int(os.getenv("FTS_HUMAN_LOGS","1"))); HUMAN_DEDUP=bool(int(os.getenv("FTS_HUMAN_DEDUP","1")))
...
class _HumanLog:
    ...
    RULES = [
        (_re.compile(r"ORDER EVENT action=config_migrated.*", _re.I),
         lambda m: ("Конфиг обновлён до новой схемы — старые настройки сохранены.", "")),
        (_re.compile(r"ORDER EVENT action=new_order.*?oid=([^\s]+).*?qty=([^\s]+)", _re.I),
         lambda m: (f"Новый заказ #{m.group(1)} найден, количество: {m.group(2)}⭐.", m.group(1))),
        (_re.compile(r"ORDER EVENT action=paid_message.*?oid=([^\s]+).*?qty=([^\s]+)", _re.I),
         lambda m: (f"Получено системное сообщение об оплате заказа #{m.group(1)} на {m.group(2)}⭐.", m.group(1))),
        (_re.compile(r"ORDER EVENT action=username_received.*?oid=([^\s]+).*?username=@?([A-Za-z0-9_]{5,32})", _re.I),
         lambda m: (f"Получен ник @{m.group(2)} для заказа #{m.group(1)}.", m.group(1))),
        (_re.compile(r"ORDER EVENT action=confirm_start.*?oid=([^\s]+).*?qty=([^\s]+).*?username=@?([A-Za-z0-9_]{5,32})", _re.I),
         lambda m: (f"Подтверждение заказа #{m.group(1)}: {m.group(2)}⭐ на @{m.group(3)}.", m.group(1))),
        (_re.compile(r"ORDER EVENT action=send_ok.*?oid=([^\s]+).*?qty=([^\s]+).*?username=@?([A-Za-z0-9_]{5,32})", _re.I),
         lambda m: (f"Заказ #{m.group(1)} выполнен: {m.group(2)}⭐ отправлены @{m.group(3)}.", m.group(1))),
        (_re.compile(r"ORDER EVENT action=send_fail.*?oid=([^\s]+).*?username=@?([A-Za-z0-9_]{5,32})", _re.I),
         lambda m: (f"Ошибка отправки по заказу #{m.group(1)} для @{m.group(2)}.", m.group(1))),
...
_SEND_EXECUTOR = ThreadPoolExecutor(
    max_workers=int(os.getenv("FTS_SEND_WORKERS", "2")),
...
def _schedule_job(key: str, fn, *args, **kwargs) -> bool:
    with _ACTIVE_JOBS_LOCK:
        ...
        return True
_JOB_BUSY_NOTIFIED: Dict[str, float] = {}
def _schedule_confirm_send(cardinal: "Cardinal", chat_id: Any, oid: Optional[str] = None, *, notify_busy: bool = True) -> bool:
    ...
    ok = _schedule_job(job_key, _run)
    if (not ok) and notify_busy:
        now = time.time()
        if now - _JOB_BUSY_NOTIFIED.get(job_key, 0) > 20:
            _JOB_BUSY_NOTIFIED[job_key] = now
            try: _safe_send(cardinal, chat_id, "⏳ Отправка уже выполняется в фоне. Cardinal не блокируется, дождитесь результата.")
            except Exception: pass
    return ok
def _log(level: str, msg: str):
    if level == "info":
        ...
        logger.debug(f"{msg}")
def _short_log_value(v: Any, limit: int = 140) -> str:
    try:
        s = str(v)
    except Exception:
        s = repr(v)
    ...
    if len(s) > limit:
        return s[:limit - 1] + "…"
    return s
def _order_log(level: str, action: str, oid: Any = None, chat_id: Any = None, qty: Any = None, username: Any = None, **extra) -> None:
...
def _s64(x: str) -> str: return _b64.b64decode(x.encode()).decode("utf-8")
NAME        = "FTS-Plugin"
VERSION     = "1.7.1"
...
_CFG_KNOWN_KEYS = {
    "plugin_enabled", "lots_active", "auto_refund", "auto_deactivate", "preorder_username",
    "unit_star_price", "markup_percent", "fragment_jwt", "wallet_version", "balance_ton",
    "balance_usdt", "last_wallet_raw", "templates", "category_id", "min_balance_ton",
    "min_balance_usdt", "star_lots", "retry_liteserver", "auto_send_without_plus",
    "skip_username_check", "queue_mode", "queue_timeout_sec", "stars_currency",
    "usdt_fallback_to_ton", "price_change_notifications", "auto_price_fragment_enabled",
    "autodump_enabled", "autodump_interval_sec", "autodump_notifications",
    "balance_lot_filter_enabled", "balance_lot_filter_notifications", "last_auto_deact_reason",
    "managed_lot_ids", "last_lot_toggle_report", "last_lot_toggle_ts",
    "order_watch_enabled", "order_watch_interval_sec", "order_wait_reminder_sec",
    "order_review_reminder_enabled", "order_review_reminder_sec", "order_records", "last_order_watch_ts",
    "last_usdt_fallback_reason", "last_usdt_fallback_ts", "last_auto_price_base_unit",
    "last_auto_price_ts", "autodump_last_ts", "config_version",
}
...
def _as_int(v: Any, default: int, min_value: Optional[int] = None, max_value: Optional[int] = None) -> int:
    try:
        if isinstance(v, bool): raise ValueError()
        if isinstance(v, str):
            m = _re.search(r"-?\d+", v.strip())
    ...
    return n
def _as_float_cfg(v: Any, default: Optional[float] = None, min_value: Optional[float] = None) -> Optional[float]:
    try:
    ...
    if isinstance(tpls, dict):
        for k, v in tpls.items():
            if k in base and isinstance(v, str) and v.strip():
                base[k] = v
    return base
def _sanitize_star_lots(items: Any) -> list:
    result: list[dict] = []
    seen: set[tuple[int, int]] = set()
    def add(qty: Any, lot_id: Any, active: Any = True, extra: Optional[dict] = None):
        q = _as_int(qty, 0, 0)
...
def _sanitize_lot_ids(items: Any) -> list[int]:
    ids: set[int] = set()
    def add(v: Any) -> None:
        try:
            lid = _as_int(v, 0, 1)
    ...
    if isinstance(items, dict):
        ...
        add(items)
    return sorted(ids)
def _sanitize_order_records(items: Any) -> dict:
    ...
    for oid, rec in list(items.items())[-ORDER_RECORDS_LIMIT:]:
        ...
        r = dict(rec); r["oid"] = str(r.get("oid") or oid)
...
def _sanitize_cfg(raw: Any, chat_id: Any = None) -> dict:
    ...
    return cfg
def _migrate_settings_data(data: Any) -> tuple[dict, bool, list[str]]:
    notes: list[str] = []
    changed = False
...
def _attach_legacy_cfg_if_needed(data: dict, key: str, cfg: Any) -> tuple[dict, bool]:
    ...
    if isinstance(legacy, dict) and legacy:
        ...
        try: _order_log("info", "config_migrated", chat_id=key, schema=SETTINGS_SCHEMA_VERSION, source="legacy")
        except Exception: pass
...
def _get_cfg(chat_id: Any) -> dict:
    ...
    if changed:
        _save_settings(data)
    return cfg
def _cfg_key_for_orders(chat_id: Any) -> str:
    key = str(chat_id)
    try:
        data = _load_settings()
        cfg = data.get(key) or {}
        if isinstance(cfg, dict) and cfg.get("fragment_jwt"): return key
        legacy = data.get(LEGACY_SETTINGS_KEY)
        if isinstance(legacy, dict) and legacy.get("fragment_jwt"): return LEGACY_SETTINGS_KEY
        for k, v in data.items():
            if k == SETTINGS_META_KEY: continue
            if isinstance(v, dict) and v.get("fragment_jwt"): return str(k)
    except Exception as e:
        logger.warning(f"_cfg_key_for_orders failed: {e}")
    return key
def _get_cfg_for_orders(chat_id: Any) -> dict:
    cfg = _get_cfg(chat_id)
    if cfg.get("fragment_jwt"): return cfg
    data = _load_settings()
    legacy = data.get(LEGACY_SETTINGS_KEY)
    ...
    return cfg
def _set_cfg_for_orders(chat_id: Any, **updates) -> dict:
...
def _normalize_stars_currency(v: Any) -> str:
...
def _safe_edit(bot, chat_id: Any, msg_id: int, text: str, kb=None) -> bool:
    try:
        ...
        if "message is not modified" in low:
            ...
            return True
        if ("message to edit not found" in low or
            "message can't be edited" in low or
            "there is no text in the message to edit" in low or
            "message is not a text message" in low or
        ...
        logger.debug(f"edit_message failed: {e}")
        return False
def _safe_send_tg(bot, chat_id: Any, text: str, kb=None):
    try:
...
def _settings_text(chat_id: Any) -> str:
    ...
    return (
        ...
        f"• Валюта отправки звёзд: <b>{_stars_currency_emoji(currency)} {_stars_currency_label(currency)}</b>\n"
        f"{fallback_line}"
        f"• Наценка на звёзды: <code>{cfg.get('markup_percent', 0.0)}%</code>\n"
        f"• Фильтр лотов по балансу: <b>{_state_on(cfg.get('balance_lot_filter_enabled', True))}</b>\n"
        f"{min_bal_line}"
...
def _token_text(chat_id: Any) -> str:
    ...
    return (
        ...
        "Создание токена внутри плагина отключено: старый способ авторизации удалён. "
        "Импортируйте готовый JWT из внешнего кабинета/инструмента.\n\n"
...
def _extract_username_from_order_text(text: str) -> Optional[str]:
    ...
    if m:
        cand = m.group(1)
        if _validate_username(cand) and _re.search(r"[A-Za-z]", cand): return cand
    m = _re.fullmatch(r'\s*@?([A-Za-z0-9_]{5,32})\s*[.!?,;:]*\s*', s)
    if m:
        cand = m.group(1)
        if _re.search(r"[A-Za-z]", cand): return cand
    return None
...
def _as_balance_float(v: Any) -> Optional[float]:
    try:
        ...
        return None
    return None
def _balance_from_node(node: Any) -> Optional[float]:
    if isinstance(node, dict):
        for key in ("balance", "amount", "value", "available", "free"):
            val = _as_balance_float(node.get(key))
            if val is not None: return val
    return _as_balance_float(node)
def _extract_wallet_info(data: dict) -> tuple[Optional[str], Optional[float], Optional[float]]:
    ...
    if bal_ton is None:
        for key in ("nanoton", "nanoTon", "nanotons", "balance_nano", "balanceNano"):
            if key in data:
                ...
                if v is not None:
                    bal_ton = v / 1e9 if v > 1e6 else v
                    break
    def walk_usdt(x: Any) -> Optional[float]:
        if isinstance(x, dict):
            for k, v in x.items():
                lk = str(k).lower()
                if "usdt" in lk:
...
def _check_fragment_wallet(jwt: str) -> tuple[Optional[str], Optional[float], Optional[float], Optional[dict]]:
    last_err = None
...
def _is_liteserver_transient_failure(resp_text: str, status: int = 0, resp_json: Any = None) -> bool:
    ...
    return True
def _fragment_order_status(resp_json: Any) -> str:
    if isinstance(resp_json, dict):
        for key in ("status", "state", "order_status"):
            val = resp_json.get(key)
            if val: return str(val).upper()
    return ""
def _fragment_order_id(resp_json: Any) -> Optional[str]:
    ...
    for subkey in ("data", "result", "payload"):
        ...
        if isinstance(sub, dict):
            ...
            if val: return val
    return None
def _is_balance_failure_resp(resp: Any) -> bool:
    try:
...
def _order_stars(
    ...
    try:
        ...
        return {
            ...
            "json": resp_json,
            "currency": cur,
            "order_status": status_val.upper() if status_val else None,
            "fragment_order_id": _fragment_order_id(resp_json),
        }
    except Exception as e:
        _log("error", f"SEND exception: {e}")
        return {"ok": False, "status": 0, "text": str(e), "json": None, "currency": _normalize_stars_currency(currency)}
def _send_stars_with_currency_fallback(
    cardinal: "Cardinal",
    chat_id: Any,
    cfg: dict,
    jwt: str,
    ...
    if (
        cur == FTS_CURRENCY_USDT_TON
        and not (resp or {}).get("ok")
        and _cfg_bool(cfg, "usdt_fallback_to_ton", False)
        and _is_balance_failure_resp(resp)
    ):
        try:
            _set_cfg_for_orders(
                chat_id,
                stars_currency=FTS_CURRENCY_TON,
                last_usdt_fallback_reason="USDT balance is not enough for Fragment order",
                last_usdt_fallback_ts=int(time.time()),
            )
        except Exception: pass
        ...
        resp = _order_stars_with_retry(
            ...
            show_sender=show_sender,
            retry_enabled=bool(cfg.get("retry_liteserver", LITESERVER_RETRY_DEFAULT)),
            currency=FTS_CURRENCY_TON,
        )
        resp["_currency_fallback"] = "usdt_ton->ton"
    return resp
def _send_order_result_message(cardinal: "Cardinal", chat_id: Any, qty: int, username: str, order_url: str, resp: Optional[dict] = None) -> None:
    ...
    if st in {"PENDING", "BLOCKCHAIN_SENT"}:
        ...
        _safe_send(
            cardinal,
            chat_id,
            f"✅ Fragment принял заявку на {int(qty)}⭐ для @{username.lstrip('@')}.\n"
            f"Валюта: <b>{_h(cur)}</b>. Статус: <code>{_h(st)}</code>."
            f"{extra}\n"
...
def _get_my_lots_by_category(cardinal: "Cardinal", category_id: int) -> Dict[int, Any]:
    ...
    def _extract_ids_from_sorted_map(all_map: Any) -> list[int]:
        ids: list[int] = []
        if not isinstance(all_map, dict): return ids
...
def _order_is_stars(order: Any) -> bool:
    try:
        ...
        return False
def _activate_lot(cardinal: "Cardinal", lot_id: int, trusted: bool = False) -> bool:
    try:
        lot_id = int(lot_id)
        ...
        return False
def _deactivate_lot(cardinal: "Cardinal", lot_id: int, trusted: bool = False) -> bool:
    try:
        lot_id = int(lot_id)
        is_star = _is_stars_lot(cardinal, lot_id)
        if not is_star and not trusted:
...
def _apply_star_lots_state(cardinal: "Cardinal", star_lots: List[dict], enabled: bool) -> Dict[str, List[int]]:
    ...
    return report
def _apply_category_state(
    cardinal: "Cardinal",
    category_id: int,
    enabled: bool,
    known_lot_ids: Optional[List[int]] = None,
) -> Dict[str, List[int]]:
    ...
    logger.info(
        f"[CATEGORY] apply_state: subcategory={category_id} enabled={enabled} "
        f"known={len(known)} direct={len(direct)} profile={len(profile)} total={len(lot_ids)}"
    )
    ...
    return report
def _managed_lot_ids_from_cfg(cfg: dict) -> list[int]:
    ...
    for it in (cfg or {}).get("star_lots") or []:
        try:
            ...
            pass
    return sorted(ids)
def _merge_lot_ids(*groups: Any) -> list[int]:
    ids: set[int] = set()
    for g in groups:
        ids.update(_sanitize_lot_ids(g))
    return sorted(ids)
def _ids_from_report(rep: Optional[dict]) -> list[int]:
    if not isinstance(rep, dict):
        return []
    return _merge_lot_ids(rep.get("ok") or [], rep.get("skip") or [], rep.get("err") or [])
def _lot_report_short(rep: Optional[dict]) -> str:
    if not isinstance(rep, dict):
        return "нет отчёта"
...
def _parse_fragment_error_text(response_text: str, status_code: int = 0) -> str:
    fallback="Ошибка обработки заказа."; low_text=(response_text or "").lower()
    try: data=json.loads(response_text)
    except Exception: data=None
    if "no jetton wallet" in low_text: return "USDT-кошелёк Fragment/TON не инициализирован или на нём нет USDT-jetton. Для оплаты USDT нужен входящий USDT в сети TON и небольшой запас TON на комиссию."
    if "low ton balance" in low_text: return "Недостаточно TON для комиссии сети TON. Даже при оплате USDT нужен небольшой запас TON."
    if "seqno" in low_text and ("exit code -256" in low_text or 'get method "seqno"' in low_text): return "Неверная версия кошелька или кошелёк не инициализирован. Проверьте кошелёк/JWT в личном кабинете и выполните небольшую исходящую транзакцию."
    if status_code == 429: return "Слишком много запросов. Попробуйте ещё раз через минуту."
    if status_code in (500, 502, 503, 504): return "Сервис Fragment временно недоступен. Повторите позже."
    if status_code in (401, 403): return "Нужна повторная авторизация продавца. Попробуем ещё раз чуть позже."
    if ("liteserver" in low_text) or ("lite server" in low_text): return "Сервис TON/Fragment временно недоступен. Попробуйте ещё раз позже."
    if isinstance(data, dict):
        if "username" in data: return "Неверный Telegram-тег (проверьте @username)."
        if "quantity" in data: return "Минимальное количество для покупки — 50 ⭐."
        for k in ("detail","message","error"):
            if data.get(k):
                msg=str(data[k]); ml=msg.lower()
                if "no jetton wallet" in ml: return "USDT-кошелёк Fragment/TON не инициализирован или на нём нет USDT-jetton."
                if "low ton balance" in ml: return "Недостаточно TON для комиссии сети TON."
                if "not enough" in ml or "balance" in ml: return "Недостаточно средств на кошельке Fragment."
                if "version" in ml: return "Неверная версия кошелька у продавца."
                if "username" in ml: return "Пользователь с таким @username не найден."
                return msg[:200]
        if isinstance(data.get("errors"), list):
            joined=" | ".join(str(x.get("error") or x) for x in data["errors"][:3]); jl=joined.lower()
            if "no jetton wallet" in jl: return "USDT-кошелёк Fragment/TON не инициализирован или на нём нет USDT-jetton."
            if "low ton balance" in jl: return "Недостаточно TON для комиссии сети TON."
            if "balance" in jl: return "Недостаточно средств на кошельке Fragment."
            return (joined or fallback)[:200]
        if isinstance(data.get("data"), dict):
            for k in ("error","message","detail"):
                if data["data"].get(k): return str(data["data"][k])[:200]
    elif isinstance(data, list) and data: return " | ".join(str(x) for x in data[:3])[:200]
    return fallback
def _classify_send_failure(resp_text: str, status: int, username: str, jwt: Optional[str]) -> tuple[str, str]:
    ...
    if "no jetton wallet" in low_resp: return "seller", "USDT-кошелёк Fragment/TON не инициализирован или на нём нет USDT-jetton."
    if "low ton balance" in low_resp: return "seller", "Недостаточно TON для комиссии сети TON."
    if status in (401, 403): return "seller", "Нужна повторная авторизация продавца."
    if status == 429: return "seller", "Слишком много запросов. Попробуйте ещё раз через минуту."
    if status in (500, 502, 503, 504): return "seller", "Сервис Fragment временно недоступен. Повторите позже."
    reason = _parse_fragment_error_text(resp_text, status)
    low = (reason or "").lower()
    if "seqno" in low_resp: return "seller", "Неверная версия кошелька у продавца или кошелёк не инициализирован."
    if any(t in low for t in ("username", "user not found", "not found", "invalid", "does not exist")): return "username", "Пользователь с таким @username не найден."
    if status == 400 and username and not _check_username_exists_throttled(username, jwt): return "username", "Пользователь с таким @username не найден."
    if any(t in low for t in ("balance", "not enough", "jetton", "комисс")): return "seller", reason or "Недостаточно средств на кошельке Fragment."
...
def _auto_refund_order(cardinal: "Cardinal", order_id: Any, chat_id: Any, reason: str) -> bool:
    try:
        if order_id and str(order_id) in _done_oids:
            logger.warning(f"[REFUND] skip #{order_id}: already sent/done"); return False
        cfg = _get_cfg(chat_id); rec = (cfg.get("order_records") or {}).get(str(order_id), {}) if order_id else {}
        if str((rec or {}).get("status") or "") in {"sent", "sent_pending"}:
            logger.warning(f"[REFUND] skip #{order_id}: order record status={(rec or {}).get('status')}"); return False
        ...
        return False
def _coin_unit_for_balance(cfg:dict, cur:str)->tuple[Optional[float],str]:
    cur=_normalize_stars_currency(cur); data,src=_fetch_fragment_prices(cfg.get("fragment_jwt")); unit=_fragment_unit_price(data,cur) if data is not None else None
    if unit: return float(unit), f"Fragment Prices: {unit:g} {_stars_currency_label(cur)} за 1⭐"
    rates=_tonapi_rates_rub(); rate=rates.get("usdt" if cur==FTS_CURRENCY_USDT_TON else "ton"); rub=_num(cfg.get("last_auto_price_base_unit") or cfg.get("unit_star_price"))
    if rub and rate: return float(rub)/float(rate), f"fallback: {rub:g} RUB / {rate:g} RUB"
    return None, f"не удалось получить стоимость 1⭐ в {_stars_currency_label(cur)} ({src})"
def _balance_filter_msg(rows:List[dict], cur:str, balance:float, unit:float)->str:
    ...
    for r in rows[:60]: out.append(f"• {r['qty']}⭐ LOT <code>{r['lot_id']}</code>: нужно <code>{r['need']:.6f} {_stars_currency_label(cur)}</code> → {'🟢 включён' if r['active'] else '🔴 выключен'}")
    if len(rows)>60: out.append(f"… и ещё {len(rows)-60} лот(ов)")
    return "\n".join(out)
def _apply_balance_lot_filter(cardinal:"Cardinal", chat_id:Any, cfg:dict, bal_ton:Any=None, bal_usdt:Any=None, force:bool=False)->tuple[bool,str]:
    if not force and not _cfg_bool(cfg,"balance_lot_filter_enabled",True): return False,"Фильтр по балансу выключен."
    cur=_normalize_stars_currency(cfg.get("stars_currency")); bal=bal_usdt if cur==FTS_CURRENCY_USDT_TON else bal_ton
    if bal is None: bal=cfg.get("balance_usdt" if cur==FTS_CURRENCY_USDT_TON else "balance_ton")
    if not isinstance(bal,(int,float)): return False,"Не удалось получить баланс кошелька."
    unit,info=_coin_unit_for_balance(cfg,cur)
    if not unit: return False,"Не удалось посчитать стоимость лотов: "+info
    avail=max(0.0,float(bal)*max(0.0,min(1.0,float(FTS_BALANCE_LOT_RESERVE_RATIO)))); items=cfg.get("star_lots") or []; rows=[]; changed=False
    for it in items:
        try:
            qty=int(it.get("qty")); lid=int(it.get("lot_id")); need=float(qty)*float(unit); ok=need<=avail+1e-12; was=bool(it.get("active")); filtered=bool(it.get("balance_filtered")); want=ok and (was or filtered or bool(cfg.get("lots_active")))
            it["balance_required_"+("usdt" if cur==FTS_CURRENCY_USDT_TON else "ton")]=round(need,6)
            if not ok and was:
                _deactivate_lot(cardinal,lid,trusted=True); it["active"]=False; it["balance_filtered"]=True; changed=True; rows.append({"qty":qty,"lot_id":lid,"need":need,"active":False})
            elif want and filtered and not was:
                _activate_lot(cardinal,lid,trusted=True); it["active"]=True; it["balance_filtered"]=False; changed=True; rows.append({"qty":qty,"lot_id":lid,"need":need,"active":True})
        except Exception as e: logger.debug(f"balance lot filter skip: {e}")
    if changed:
        ...
        if _cfg_bool(cfg,"balance_lot_filter_notifications",True):
            try: cardinal.telegram.bot.send_message(chat_id,_balance_filter_msg(rows,cur,float(bal),float(unit)),parse_mode="HTML")
            except Exception as e: logger.warning(f"balance filter notify failed: {e}")
...
def _maybe_auto_deactivate(cardinal: "Cardinal", cfg: dict, chat_id: Optional[Any] = None):
    ...
    if jwt:
        try:
            _set_cfg(
                owner_chat,
                wallet_version=ver,
                balance_ton=(round(bal_ton, 6) if isinstance(bal_ton, (int, float)) else None),
                balance_usdt=(round(bal_usdt, 6) if isinstance(bal_usdt, (int, float)) else None),
                last_wallet_raw=_raw
            )
        except Exception: pass
    try:
        if cardinal is not None and chat_id is not None:
            _maybe_auto_price_update(cardinal, chat_id); _maybe_autodump_update(cardinal, chat_id)
    except Exception as e: logger.debug(f"auto price update skipped: {e}")
    try:
        if cardinal is not None and chat_id is not None: _apply_balance_lot_filter(cardinal, owner_chat, _get_cfg(owner_chat), bal_ton, bal_usdt)
    except Exception as e: logger.debug(f"balance lot filter skipped: {e}")
    cur = _normalize_stars_currency(cfg.get("stars_currency"))
    if cur == FTS_CURRENCY_USDT_TON:
        ...
        if isinstance(bal_usdt, (int, float)) and bal_usdt < thr_usdt and cfg.get("auto_deactivate", False):
            ...
            _set_cfg(
                ...
                last_auto_deact_reason=f"Баланс {bal_usdt} USDT < порога {thr_usdt}",
                last_lot_toggle_report=f"auto_deactivate_usdt: {_lot_report_short(rep)}",
    ...
    if bal_ton is not None and bal_ton < thr and cfg.get("auto_deactivate", False):
        ...
        _set_cfg(
            ...
            star_lots=items,
            managed_lot_ids=managed_ids,
            last_auto_deact_reason=f"Баланс {bal_ton} TON < порога {thr}",
            last_lot_toggle_report=f"auto_deactivate_ton: {_lot_report_short(rep)}",
            last_lot_toggle_ts=int(time.time()),
        )
...
def _track_fsm_mid(state: dict, mid: Optional[int]) -> None:
    if not CLEAN_FSM_SENSITIVE or not mid: return
...
def _cleanup_fsm_msgs(bot, chat_id: Any, state: dict) -> None:
    if not CLEAN_FSM_SENSITIVE: return
...
def _settings_kb(chat_id: Any) -> InlineKeyboardMarkup:
    ...
    return kb
def _fmt_minutes_from_sec(sec: Any) -> str:
    try:
...
def _mini_settings_text(chat_id: Any) -> str:
    ...
    order_summary = (
        f"{_state_on(cfg.get('order_watch_enabled', True))}, проверка {watch_every}, "
        f"ожидание {wait_after}, отзыв {review_after}"
    )
    fb_line = ""
    ...
    return (
        "<b>Мини-настройки</b>\n\n"
        f"• Очередь: <b>{qtxt}</b>\n"
        f"• Мин. баланс {cur_min_label}: <code>{cur_min}</code>\n"
        f"• Повтор при LiteServer: <b>{retry_state}</b>\n"
        f"• Проверка @username при вводе: <b>{check_state}</b>\n"
        f"• Подтверждение «+»: <b>{plus_state}</b>\n"
        f"• Заказы и отзывы: <code>{_h(order_summary)}</code>\n"
        f"{fb_line}"
        f"• Уведомления цен: <b>{_state_on(cfg.get('price_change_notifications', True))}</b>\n"
...
def _mini_settings_kb(chat_id: Any) -> InlineKeyboardMarkup:
    cfg = _get_cfg(chat_id)
    kb = K()
    mode = _queue_mode(chat_id); timeout = _queue_timeout_sec(chat_id)
...
def _order_tools_text(chat_id: Any) -> str:
    cfg = _get_cfg(chat_id)
    watch_every = _fmt_minutes_from_sec(cfg.get("order_watch_interval_sec", ORDER_WATCH_INTERVAL_DEFAULT))
    wait_after = _fmt_minutes_from_sec(cfg.get("order_wait_reminder_sec", ORDER_WAIT_REMINDER_DEFAULT))
    review_after = _fmt_minutes_from_sec(cfg.get("order_review_reminder_sec", ORDER_REVIEW_REMINDER_DEFAULT))
    ...
    try:
        for rec in recs.values():
            ...
            if st in {"queued", "await_username", "await_confirm", "ready", "sending"}: active += 1
            if st in {"sent", "sent_pending"}: sent += 1
    except Exception:
        pass
    return (
        "<b>🧹 Заказы и отзывы</b>\n\n"
        f"• Проверка зависших заказов: <b>{_state_on(cfg.get('order_watch_enabled', True))}</b>\n"
        f"• Интервал проверки: <code>{watch_every}</code>\n"
        f"• Напоминать ожидание через: <code>{wait_after}</code>\n"
        f"• Напоминание об отзыве: <b>{_state_on(cfg.get('order_review_reminder_enabled', True))}</b>\n"
...
def _order_tools_kb(chat_id: Any) -> InlineKeyboardMarkup:
    cfg = _get_cfg(chat_id)
    kb = K()
...
def _saves_text(chat_id: Any) -> str:
    try:
    ...
    return (
        ...
        f"Текущий файл: <code>{size} байт</code>\n\n"
        "⚠️ В сохранениях может быть JWT-токен. Не отправляйте файл посторонним."
    )
def _saves_kb() -> InlineKeyboardMarkup:
    kb = K()
...
def _ask_import_saves(bot, call):
    ...
    m = bot.send_message(
        chat_id,
        "Пришлите файл <code>settings.json</code> или вставьте JSON текстом.\n"
        "Импорт заменит текущие сохранения. Для отмены: /cancel",
        parse_mode="HTML",
...
def _download_saves(bot, call):
    ...
    try:
        ...
        bot.send_document(
            chat_id,
            (fname, payload),
            caption="💾 Текущие сохранения FTS-Plugin. Файл может содержать JWT-токен."
        )
        try:
...
def _import_settings_payload(raw_text: str) -> tuple[bool, str]:
    try:
        obj = json.loads((raw_text or "").lstrip("\ufeff").strip())
        if isinstance(obj, dict) and isinstance(obj.get("settings"), dict):
            obj = obj["settings"]
        if not isinstance(obj, dict): return False, "JSON должен быть объектом."
...
def _stars_kb(chat_id: Any) -> InlineKeyboardMarkup:
    ...
    for it in (cfg.get("star_lots") or [])[:10]:
        ...
        kb.row(B(f"{qty}⭐  LOT {lot_id}  {state}", callback_data=f"{CBT_STAR_TOGGLE_P}{lot_id}"),
               B("💰 Цена", callback_data=f"{CBT_STAR_PRICE_P}{lot_id}"), B(floor_txt, callback_data=f"{CBT_AUTODUMP_FLOOR_P}{lot_id}"), B("🗑", callback_data=f"{CBT_STAR_DEL_P}{lot_id}"))
    ...
    kb.add(B("◀️ Назад", callback_data=CBT_SETTINGS))
    return kb
def _pricing_text(chat_id: Any) -> str:
    cfg = _get_cfg(chat_id); unit = cfg.get("unit_star_price"); base = cfg.get("last_auto_price_base_unit")
    unit_txt = f"{unit}" if isinstance(unit, (int, float)) else "—"; base_txt = f"{base}" if isinstance(base, (int, float)) else "—"
    return ("<b>💹 Ценообразование лотов</b>\n\n"f"• Цена за 1⭐ без наценки: <code>{base_txt}</code>\n"f"• Цена за 1⭐ с наценкой: <code>{unit_txt}</code>\n"f"• Наценка: <code>{cfg.get('markup_percent', 0.0)}%</code>\n"f"• Автоизменение цен: <b>{_state_on(cfg.get('auto_price_fragment_enabled', False))}</b>\n"f"• Автодемп: <b>{_state_on(cfg.get('autodump_enabled', False))}</b>\n"f"• Фильтр по балансу: <b>{_state_on(cfg.get('balance_lot_filter_enabled', True))}</b>\n"f"• Валюта отправки звёзд: <b>{_stars_currency_emoji(cfg.get('stars_currency'))} {_stars_currency_label(cfg.get('stars_currency'))}</b>\n\n""")
def _pricing_kb(chat_id: Any = None) -> InlineKeyboardMarkup:
    cfg = _get_cfg(chat_id) if chat_id is not None else {}; kb = K()
    ...
    kb.add(B("◀️ Назад", callback_data=CBT_STARS)); return kb
def _open_pricing(bot, call):
    chat_id = call.message.chat.id
    _safe_edit(bot, chat_id, call.message.id, _pricing_text(chat_id), _pricing_kb(chat_id))
    try: bot.answer_callback_query(call.id)
    except Exception: pass
def _autodump_text(chat_id: Any) -> str:
    cfg=_get_cfg(chat_id); interval=int(cfg.get("autodump_interval_sec",1800)); mins=interval//60
    ...
    last=cfg.get("last_autodump_info") or "—"; qtxt=", ".join(map(str, qtys[:30])) if qtys else "—"; ftxt=", ".join(floors[:20]) if floors else "—"
    return ("<b>📉 Автодемп</b>\n\n"f"• Состояние: <b>{_state_on(cfg.get('autodump_enabled', False))}</b>\n"f"• Интервал проверки: <code>{mins} мин.</code>\n"f"• Уведомления: <b>{_state_on(cfg.get('autodump_notifications', True))}</b>\n"f"• Количества для проверки: <code>{qtxt}</code>\n"f"• Пороги демпа: <code>{_h(ftxt)}</code>\n"f"• Последний результат: <code>{_h(last)}</code>\n\n""Автодемп ищет цены конкурентов по вашим количествам ⭐. Если точного количества нет, берёт ближайшее и досчитывает недостающие звёзды. Порог не даёт опустить цену ниже заданной.")
def _autodump_kb(chat_id: Any) -> InlineKeyboardMarkup:
    cfg=_get_cfg(chat_id); kb=K()
    ...
    kb.add(B("◀️ Назад", callback_data=CBT_PRICING)); return kb
def _open_autodump(bot, call):
    chat_id=call.message.chat.id; _safe_edit(bot, chat_id, call.message.id, _autodump_text(chat_id), _autodump_kb(chat_id))
    try: bot.answer_callback_query(call.id)
    except Exception: pass
def _notifications_text(chat_id: Any) -> str:
    cfg = _get_cfg(chat_id)
...
def _notifications_kb(chat_id: Any) -> InlineKeyboardMarkup:
    cfg = _get_cfg(chat_id); kb = K()
    ...
    kb.add(B("◀️ Назад", callback_data=CBT_MINI_SETTINGS)); return kb
...
def _deactivate_all_star_lots(cardinal: "Cardinal", cfg: dict, chat_id: Any, reason: str = "временная ошибка/невалидный заказ") -> None:
    try:
        ...
        _set_cfg(
            ...
            star_lots=items,
            managed_lot_ids=managed_ids,
            last_auto_deact_reason=reason,
            last_lot_toggle_report=f"auto_deactivate: {_lot_report_short(rep)}",
            last_lot_toggle_ts=int(time.time()),
        )
    except Exception as e:
...
def _lots_state_summary(cfg: dict) -> tuple[str, Optional[bool]]:
    ...
    if items:
        ...
        if on == 0: return ("🔴 Выключены", False)
        if on == total: return ("🟢 Включены", True)
...
def _fetch_fragment_prices(jwt: Optional[str]) -> tuple[Optional[dict], str]:
    headers = {"Accept": "application/json"}
    if jwt: headers["Authorization"] = f"JWT {jwt}"
    last = ""
    for url in FRAGMENT_PRICES_URLS:
        try:
            ...
            if r.status_code < 400:
                ...
                except Exception: return {"raw": r.text}, url
            last = f"{url}: HTTP {r.status_code} {r.text[:160]}"
        except Exception as e: last = f"{url}: {e}"
    return None, last
def _price_nodes(x: Any):
    if isinstance(x, dict):
        yield x
        for v in x.values(): yield from _price_nodes(v)
    elif isinstance(x, (list, tuple)):
        for v in x: yield from _price_nodes(v)
def _qty_from_node(d: dict) -> Optional[float]:
    for k in ("quantity","qty","stars","count","amount_stars","stars_count"):
        v = _num(d.get(k))
        if v and v > 0: return v
    return None
def _fragment_unit_price(data: Any, currency: str) -> Optional[float]:
    cur = _normalize_stars_currency(currency); best = None
    for d in _price_nodes(data):
        q = _qty_from_node(d); vals = []
        for k, v in d.items():
            lk = str(k).lower()
            if "fee" in lk or "commission" in lk: continue
            n = _num(v)
            if n is None or n <= 0: continue
            if cur == FTS_CURRENCY_USDT_TON and (("usdt" in lk and "price" in lk) or str(d.get("currency","")).lower() in ("usdt","usdt_ton") and lk in ("price","amount","cost","total")): vals.append(n)
            if cur == FTS_CURRENCY_TON and (("ton" in lk and "price" in lk and "usdt" not in lk) or str(d.get("currency","")).lower() == "ton" and lk in ("price","amount","cost","total")): vals.append(n)
        for price in vals:
            unit = price / q if q else price
            if unit > 0 and (best is None or unit < best): best = unit
    return best
def _tonapi_rates_rub() -> dict:
    hdr = {"Accept": "application/json"}
    if TONAPI_KEY: hdr["Authorization"] = f"Bearer {TONAPI_KEY}"
    ...
    def usd_rub():
        for url, params, path in (
            ("https://api.coingecko.com/api/v3/simple/price", {"ids":"tether", "vs_currencies":"rub"}, ("tether","rub")),
            ("https://open.er-api.com/v6/latest/USD", None, ("rates","RUB")),
        ):
            try:
                r = _HTTP.get(url, params=params, headers={"Accept":"application/json"}, timeout=10)
                data = r.json()
                cur = data
                for k in path: cur = cur.get(k) if isinstance(cur, dict) else None
                n = val(cur)
                if n: return n
            except Exception: pass
        return None
    def from_rates(data, aliases):
        ...
        if isinstance(rr, dict):
            for key, node in rr.items():
                blob = str(key).lower()
                if not any(a in blob for a in aliases): continue
...
def _auto_unit_price_rub(cfg: dict) -> tuple[Optional[float], str, Optional[float]]:
    data, src = _fetch_fragment_prices(cfg.get("fragment_jwt"))
    if data is None: return None, f"Не удалось получить Fragment Prices: {src}", None
    cur = _normalize_stars_currency(cfg.get("stars_currency")); unit = _fragment_unit_price(data, cur)
    if not unit: return None, "Не смог найти цену звёзд в ответе Fragment Prices. Проверьте FRAGMENT_PRICES_URL.", None
    rates = _tonapi_rates_rub(); rub = rates.get("usdt" if cur == FTS_CURRENCY_USDT_TON else "ton")
    if not rub: return None, "Не удалось получить курс RUB через TonAPI и резервные источники. Проверьте интернет/FTS_TONAPI_KEY или задайте цену за 1⭐ вручную.", None
    base = float(unit) * float(rub); markup_percent = float(cfg.get("markup_percent") or 0.0); final = base * (1.0 + markup_percent / 100.0)
...
def _price_changes_text(rows: List[dict], title: str) -> str:
    out = [f"<b>{title}</b>"]
    for r in rows[:60]:
        diff = r.get("diff", 0); sign = "+" if float(diff or 0) >= 0 else ""
    ...
    if len(rows) > 60: out.append(f"… и ещё {len(rows)-60} лот(ов)")
    return "\n".join(out)
def _notify_price_changes(cardinal: "Cardinal", chat_id: Any, rows: List[dict], title: str):
    if rows and _cfg_bool(_get_cfg(chat_id), "price_change_notifications", True):
        try: cardinal.telegram.bot.send_message(chat_id, _price_changes_text(rows, title), parse_mode="HTML")
        except Exception as e: logger.warning(f"price notify failed: {e}")
def _apply_auto_prices(cardinal: "Cardinal", chat_id: Any, manual: bool = False) -> tuple[bool, str]:
    cfg = _get_cfg(chat_id); unit, info, base_unit = _auto_unit_price_rub(cfg)
    if not unit: return False, f"⚠️ Автообновление не удалось.\n{_h(info)}"
    rows, skipped = _collect_unit_price_targets(cardinal, cfg, unit, base_unit)
    if not rows: return False, f"⚠️ Лотов для обновления не найдено. Пропущено: {skipped}"
...
def _maybe_auto_price_update(cardinal: "Cardinal", chat_id: Any, force: bool = False) -> bool:
    cfg = _get_cfg(chat_id)
    if not force and not _cfg_bool(cfg, "auto_price_fragment_enabled", False): return False
    last = int(float(cfg.get("last_auto_price_ts") or 0))
    if not force and time.time() - last < FTS_AUTO_PRICE_INTERVAL_SEC: return False
    ok, msg = _apply_auto_prices(cardinal, chat_id, manual=False)
    if not ok: logger.warning(msg.replace("\n", " "))
    return ok
def _cb_auto_price_fragment(cardinal: "Cardinal", call):
    bot = cardinal.telegram.bot; chat_id = call.message.chat.id
    try: bot.answer_callback_query(call.id, "Считаю цены по Fragment/TonAPI…")
    except Exception: pass
    ok, msg = _apply_auto_prices(cardinal, chat_id, manual=True)
    bot.send_message(chat_id, msg, parse_mode="HTML")
    try: _open_pricing(bot, call)
    except Exception: pass
def _iter_lot_like(x):
    if x is None: return
    if isinstance(x, dict):
        if any(k in x for k in ("price","cost","amount","price_rub","title","name","tc_price")): yield x
        for v in x.values():
            if isinstance(v,(list,tuple,dict,set)): yield from _iter_lot_like(v)
    elif isinstance(x,(list,tuple,set)):
        for v in x: yield from _iter_lot_like(v)
    else: yield x
def _lot_attr(o, names, default=None):
    if isinstance(o, dict): return next((o.get(n) for n in names if n in o), default)
    for n in names:
        if hasattr(o,n):
            try: return getattr(o,n)
            except Exception: pass
    return default
def _lot_url(o) -> Optional[str]:
    val=_lot_attr(o,("url","link","href","public_url","lot_url","offer_url"),None)
    if val: return str(val)
    lid=_lot_attr(o,("id","lot_id","offer_id"),None)
    try:
        if lid: return f"https://funpay.com/lots/offer?id={int(lid)}"
    except Exception: pass
    return None
def _lot_price_qty(o) -> tuple[Optional[float], Optional[int], str]:
    price = _num(_lot_attr(o,("price","cost","amount","price_rub","tc_price","total")))
    if isinstance(_lot_attr(o,("price",),None), dict): price = _balance_from_node(_lot_attr(o,("price",),None))
...
def _html_lot_id_from_href(href: Optional[str]) -> Optional[int]:
    if not href: return None
    try:
        m = _re.search(r"(?:id=|offer=|/)(\d{2,})", str(href))
        return int(m.group(1)) if m else None
    except Exception:
        return None
def _extract_price_from_lot_html(chunk: str, plain: Optional[str] = None) -> Optional[float]:
    plain = plain if plain is not None else _html.unescape(_re.sub(r"<[^>]+>", " ", chunk or ""))
    patterns = (
        r'(?:data-s|data-price|data-cost|data-value)=["\']([^"\']+)["\']',
        r'class=["\'][^"\']*(?:tc-price|price)[^"\']*["\'][^>]*>(.{0,160}?)(?:</|$)',
        r'(\d[\d\s\u00a0\u202f.,]{0,16})\s*(?:₽|руб\.?|RUB)\b',
    )
    for pat in patterns:
        vals = _re.findall(pat, chunk or plain, _re.I | _re.S)
        for val in reversed(vals):
            v = _num(_html.unescape(_re.sub(r"<[^>]+>", " ", str(val))))
            if v is not None and v > 0:
                return float(v)
    return None
def _funpay_http_lots(cardinal:"Cardinal", category_id:int):
    sess=_HTTP; acc=getattr(cardinal,"account",None)
    for s in [getattr(obj,n,None) for obj in (acc,cardinal) if obj for n in ("session","_session","requests","requester")]:
        if hasattr(s,"get"): sess=s
    lots=[]
    urls = (
        f"https://funpay.com/lots/{int(category_id)}/",
        f"https://funpay.com/chips/{int(category_id)}/",
    )
    for url in urls:
        try:
            ...
            if not html or status>=400:
                ...
                continue
            chunks=_re.split(r'(?=<[^>]+class=["\'][^"\']*(?:tc-item|offer|lot|tc-service)[^"\']*["\'])',html) or []
            if len(chunks)<2:
                chunks=_re.findall(r'(?is)<a[^>]+href=["\'][^"\']*(?:lots|chips|offers|orders)[^"\']+["\'].{0,9000}?</a>',html)
            for ch in chunks:
                plain=_html.unescape(_re.sub(r"<[^>]+>"," ",ch))
                qty=_extract_qty_from_title(plain)
                if not qty: continue
                val=_extract_price_from_lot_html(ch, plain)
                href=None
                hm=_re.search(r'href=["\']([^"\']+)["\']',ch,_re.I)
                if hm:
                    href=_html.unescape(hm.group(1))
                    if href.startswith("/"): href="https://funpay.com"+href
        ...
        except Exception as e: logger.debug(f"FunPay HTML lots failed {url}: {e}")
    logger.debug(f"FunPay HTML lots parsed: category={category_id} count={len(lots)}")
    return lots
def _public_category_lots(cardinal:"Cardinal", category_id:int):
    acc=getattr(cardinal,"account",None); objs=[acc,cardinal,getattr(cardinal,"profile",None),getattr(cardinal,"tg_profile",None)]; lots=[]
    ...
    for obj in objs:
        ...
        for name in names:
            ...
            for args in ((int(category_id),),("lot",int(category_id)),(int(category_id),"lot"),(int(category_id),1),(1,int(category_id)),()):
                try:
                    res=fn(*args)
                    if res: lots.extend(list(_iter_lot_like(res)))
    ...
    return lots
def _competitor_star_prices(cardinal:"Cardinal", cfg:dict, return_debug: bool=False):
    qtys=sorted({int(x.get("qty")) for x in (cfg.get("star_lots") or []) if x.get("qty")})
    ...
    byq={}
    for c in cand: byq.setdefault(c["qty"],[]).append(c)
    for target in qtys:
        pool=byq.get(target); approx=False; src=None
        if not pool:
            lower=[q for q in byq if q<=target]
            src=max(lower) if lower else (min(byq.keys(),key=lambda q:abs(q-target)) if byq else None)
            pool=byq.get(src,[]) if src else []; approx=True
    ...
    return (out, dbg) if return_debug else out
def _autodump_target_price(price:float, currency:Any) -> float:
    cur=getattr(currency,"name",str(currency)).upper(); step=FTS_AUTODUMP_STEP_RUB if cur in ("RUB","RUR","₽") else 0.01; return max(step, round(float(price)-step, 2))
def _autodump_changes_text(rows:List[dict], title:str)->str:
    out=[f"<b>{title}</b>"]
    for r in rows[:60]:
        link=""
        ...
        if r.get("approx"):
            ...
            if int(r.get("added_qty") or 0)>0: extra+=f" + {int(r.get('added_qty'))}⭐"
            extra+=")"
    ...
    if len(rows)>60: out.append(f"… и ещё {len(rows)-60} лот(ов)")
    return "\n".join(out)
def _notify_autodump_changes(cardinal:"Cardinal", chat_id:Any, rows:List[dict], title:str):
    if rows and _cfg_bool(_get_cfg(chat_id),"autodump_notifications",True):
        try: cardinal.telegram.bot.send_message(chat_id,_autodump_changes_text(rows,title),parse_mode="HTML")
        except Exception as e: logger.warning(f"autodump notify failed: {e}")
def _apply_autodump(cardinal:"Cardinal", chat_id:Any, manual:bool=False)->tuple[bool,str]:
    cfg=_get_cfg(chat_id); comp, dbg=_competitor_star_prices(cardinal,cfg,return_debug=True)
    if not comp:
        info=(f"лоты={dbg.get('raw_lots',0)}, кандидаты={dbg.get('candidates',0)}, "
              f"свои={dbg.get('self_skipped',0)}, без цены/кол-ва={dbg.get('bad_price_or_qty',0)}, "
              f"количества={','.join(map(str, dbg.get('target_qtys') or [])) or '—'}")
        _set_cfg(chat_id,last_autodump_ts=int(time.time()),last_autodump_info="цены конкурентов не найдены: "+info)
        return False,("⚠️ Автодемп не нашёл подходящие цены конкурентов.\n"
                      f"Диагностика: <code>{_h(info)}</code>\n"
                      "Плагин проверил методы Cardinal/FunPayAPI и HTML-страницу FunPay. "
                      "Если в категории реально есть конкуренты, проверьте доступность публичной страницы FunPay из сервера и правильность категории звёзд.")
    rows=[]; fair_unit=fair_base=fair_info=None; tried_fair=False
    for it in (cfg.get("star_lots") or []):
        try:
            qty=int(it.get("qty")); lot_id=int(it.get("lot_id")); c=comp.get(qty)
            if not c: continue
            old, cur=_get_lot_price_currency(cardinal,lot_id)
            if old is None: continue
            target=_autodump_target_price(c["price"],cur); floor=_num(it.get("autodump_min_price")); decision=""
            if floor is not None and floor>0 and target<floor:
                target=float(floor); decision="сработал порог демпа"
            if target>float(old)+0.01:
                if not tried_fair:
                    fair_unit,fair_info,fair_base=_auto_unit_price_rub(cfg); tried_fair=True
                if fair_unit:
                    fair_price=float(fair_unit)*qty; cur_name=getattr(cur,"name",str(cur)).upper()
                    ...
                    continue
            if abs(float(target)-float(old))<0.01: continue
    ...
    rep=_apply_markup_prices(cardinal,rows); _set_cfg(chat_id,last_autodump_ts=int(time.time()),last_autodump_info=f"обновлено {len(rep['ok'])}/{len(rows)}")
...
def _maybe_autodump_update(cardinal:"Cardinal", chat_id:Any, force:bool=False)->bool:
    cfg=_get_cfg(chat_id)
    if not force and not _cfg_bool(cfg,"autodump_enabled",False): return False
    last=int(float(cfg.get("last_autodump_ts") or 0)); interval=max(600,min(86400,int(float(cfg.get("autodump_interval_sec") or FTS_AUTODUMP_DEFAULT_INTERVAL_SEC))))
    if not force and time.time()-last<interval: return False
...
def _collect_markup_targets(cardinal: "Cardinal", cfg: dict, percent: float) -> List[dict]:
    ...
    for lot_id in lot_ids:
        ...
        seen.add(lot_id)
        try:
            if not _is_stars_lot(cardinal, lot_id): continue
            fields = cardinal.account.get_lot_fields(int(lot_id))
            if not fields: continue
...
def _collect_reset_markup_targets(cardinal: "Cardinal", cfg: dict, percent: float) -> List[dict]:
    if abs(float(percent or 0.0)) < 1e-12: return []
    rows: List[dict] = []
    star_lots = cfg.get("star_lots") or []
    if star_lots:
    ...
    for lot_id in lot_ids:
        ...
        seen.add(lot_id)
        try:
            if _CARDINAL_REF and not _is_stars_lot(_CARDINAL_REF, lot_id): continue
...
def _get_lot_price_currency(cardinal: "Cardinal", lot_id: int) -> tuple[Optional[float], str]:
    try:
        ...
        return None, "RUB"
def _collect_unit_price_targets(cardinal: "Cardinal", cfg: dict, unit_price: float, base_unit_price: Optional[float] = None) -> tuple[List[dict], int]:
    rows: List[dict] = []
    skipped = 0
    star_lots = cfg.get("star_lots") or []
    if star_lots:
    ...
    for lot_id in lot_ids:
        ...
        try:
            ...
            if curr_name in ("RUB", "RUR", "₽"):
                if base_price is not None: base_price = float(int(round(base_price)))
                new_price = float(int(round(new_price)))
            else:
                if base_price is not None: base_price = round(base_price, 2)
...
def _cb_unit_price_apply(cardinal: "Cardinal", call):
    ...
    try: _open_pricing(cardinal.telegram.bot, call)
    except Exception: pass
...
def _order_record_update(chat_id: Any, oid: Optional[Any], **updates) -> None:
    if not oid: return
    try:
        oid_s = str(oid); cfg = _get_cfg(chat_id); recs = dict(cfg.get("order_records") or {})
        rec = dict(recs.get(oid_s) or {}); now = int(time.time())
        rec.setdefault("oid", oid_s); rec.setdefault("chat_id", str(chat_id)); rec.setdefault("created_ts", now)
        rec.update({k: v for k, v in updates.items() if v is not None}); rec["updated_ts"] = now; recs[oid_s] = rec
        if len(recs) > ORDER_RECORDS_LIMIT:
            old = sorted(recs.items(), key=lambda kv: int((kv[1] or {}).get("updated_ts") or 0)); recs = dict(old[-ORDER_RECORDS_LIMIT:])
        _set_cfg(chat_id, order_records=recs)
    except Exception as e: logger.debug(f"order record update failed: {e}")

def _resp_indicates_delivery(resp: Any) -> bool:
    if not isinstance(resp, dict): return False
    if resp.get("ok"): return True
    status = str(resp.get("order_status") or _fragment_order_status(resp.get("json")) or "").upper()
    if status in {"PENDING", "BLOCKCHAIN_SENT", "OK", "SUCCESS", "COMPLETED", "COMPLETE", "DONE"}: return True
    if resp.get("fragment_order_id"): return True
    raw = resp.get("json")
    try: dump = json.dumps(raw, ensure_ascii=False).lower() if isinstance(raw, (dict, list)) else str(raw or "").lower()
    except Exception: dump = str(raw or "").lower()
    combined = dump + " " + str(resp.get("text") or "").lower()
    if any(bad in combined for bad in ("user not found", "invalid username", "not enough", "insufficient", "low ton balance", "no jetton wallet")): return False
    return any(x in combined for x in ("blockchain_sent", "pending", "transaction_id", "transactionid", "tx_hash", '"tx"', "stars_order"))

def _is_pending_delivery(resp: Any) -> bool:
    st = str((resp or {}).get("order_status") or _fragment_order_status((resp or {}).get("json")) or "").upper()
    return st in {"PENDING", "BLOCKCHAIN_SENT"}

def _should_auto_refund(cfg: dict, oid: Any, resp: Any = None, reason: str = "") -> bool:
    if not (cfg.get("auto_refund", False) and oid): return False
    if str(oid) in _done_oids: logger.warning(f"[REFUND] skip #{oid}: order already marked as sent"); return False
    if _resp_indicates_delivery(resp): logger.warning(f"[REFUND] skip #{oid}: Fragment response looks delivered/pending"); return False
    low = (reason or "").lower()
    if "pending" in low or "blockchain" in low or "заявк" in low or "принял" in low: logger.warning(f"[REFUND] skip #{oid}: reason looks pending/delivered: {reason}"); return False
    return True

def _mark_order_sent_record(chat_id: Any, oid: Optional[Any], qty: int, username: str, resp: Optional[dict]) -> None:
    if not oid: return
    _order_record_update(chat_id, oid, status=("sent_pending" if _is_pending_delivery(resp) else "sent"), qty=int(qty or 0), username=str(username or "").lstrip("@"), sent_ts=int(time.time()), fragment_status=(resp or {}).get("order_status"), fragment_id=(resp or {}).get("fragment_order_id"), review_reminder_ts=0)

def _order_health_check(cardinal: "Cardinal", chat_id: Any, *, manual: bool = False) -> tuple[int, int, int]:
    cfg = _get_cfg(chat_id)
    if not manual and not _cfg_bool(cfg, "order_watch_enabled", True): return (0, 0, 0)
    now = time.time(); wait_sent = review_sent = fixed = 0
    wait_sec = int(cfg.get("order_wait_reminder_sec") or ORDER_WAIT_REMINDER_DEFAULT); review_sec = int(cfg.get("order_review_reminder_sec") or ORDER_REVIEW_REMINDER_DEFAULT)
    for item in list(_q(chat_id)):
        try:
            if item.get("finalized") or not _allowed_stages(item): continue
            cid = item.get("chat_id") or chat_id; stage = str(item.get("stage") or "")
            ts = float(item.get("stage_ts") or item.get("turn_ts") or item.get("created_ts") or now); last = float(item.get("last_reminder_ts") or 0)
            if now - ts >= wait_sec and now - last >= wait_sec:
                qty = int(item.get("qty") or 50); oid = item.get("order_id") or "—"
                ...
                _safe_send(cardinal, cid, msg); item["last_reminder_ts"] = now; wait_sent += 1
        ...
        except Exception as e: logger.debug(f"order watch queue item skipped: {e}")
    if _cfg_bool(cfg, "order_review_reminder_enabled", True):
        recs = dict(cfg.get("order_records") or {}); changed = False
        for oid, rec in list(recs.items()):
            try:
                if str(rec.get("status") or "") not in {"sent", "sent_pending"}: continue
                if int(rec.get("review_reminder_ts") or 0) > 0: continue
                sent_ts = int(rec.get("sent_ts") or rec.get("finalized_ts") or 0)
                if sent_ts <= 0 or now - sent_ts < review_sec: continue
                cid = rec.get("chat_id") or chat_id; qty = int(rec.get("qty") or 0); uname = str(rec.get("username") or "").lstrip("@")
                ...
                rec["review_reminder_ts"] = int(now); rec["updated_ts"] = int(now); recs[oid] = rec; changed = True; review_sent += 1
            except Exception as e: logger.debug(f"review reminder skipped: {e}")
        if changed: _set_cfg(chat_id, order_records=recs)
    try: _set_cfg(chat_id, last_order_watch_ts=int(now))
    except Exception: pass
    return (wait_sent, review_sent, fixed)

_AUTO_MAINT_STARTED=False
def _start_auto_maintenance(cardinal:"Cardinal"):
    global _AUTO_MAINT_STARTED
    if _AUTO_MAINT_STARTED: return
            ...
            try:
                ...
                for cid,cfg in list(data.items()):
                    if cid in (SETTINGS_META_KEY, LEGACY_SETTINGS_KEY):
                        continue
                    if isinstance(cfg,dict) and (cfg.get("auto_price_fragment_enabled") or cfg.get("autodump_enabled") or cfg.get("order_watch_enabled") or cfg.get("order_review_reminder_enabled")):
                        _maybe_auto_price_update(cardinal,cid); _maybe_autodump_update(cardinal,cid)
                        try:
                            last=int(cfg.get("last_order_watch_ts") or 0); interval=int(cfg.get("order_watch_interval_sec") or ORDER_WATCH_INTERVAL_DEFAULT)
                            if time.time()-last >= max(60, interval): _order_health_check(cardinal,cid)
                        except Exception as e: logger.debug(f"order health check skipped: {e}")
            except Exception as e: logger.debug(f"auto maintenance skipped: {e}")
            time.sleep(60)
...
def init_cardinal(cardinal: Cardinal):
    ...
    fsm_steps = {
        ...
        "set_autodump_interval",
        "set_order_watch_interval", "set_order_wait_reminder", "set_review_reminder_time",
    }
    tg.msg_handler(
...
def _open_settings(bot, call):
    ...
    try:
        ...
        try: bot.answer_callback_query(call.id, "Не удалось открыть настройки. Проверьте лог плагина.", show_alert=True)
        except Exception: pass
        try:
...
def _stats_collect(lines: list[str], since_ts: Optional[float]) -> dict:
    ...
    for ln in lines:
        ...
        if since_ts and ts and ts<since_ts: continue
        low=ln.lower()
        ...
        if "цены лотов обновлены" in low or "автоцены применены" in low: stats["price_updates"]+=1
        if "автодемп" in low and ("изменил" in low or "применён" in low): stats["autodump_updates"]+=1
        m=_re.search(r"SEND start:\s*(\d+)\s*⭐\s*→\s*@?([A-Za-z0-9_]{4,32})(?:.*?currency=([a-z0-9_]+))?",ln)
        if m:
            last={"qty":int(m.group(1)),"user":m.group(2).lower(),"cur":(m.group(3) or "ton"),"ts":ts}; continue
        m=_re.search(r"SEND OK\s+(\d+)\s*⭐.*?@([A-Za-z0-9_]{4,32})",ln)
        if m:
            q=int(m.group(1)); u=m.group(2).lower(); cur=(last or {}).get("cur","ton"); stats["ok"]+=1; stats["qty_ok"]+=q; stats["per_user"][u]["qty"]+=q; stats["per_user"][u]["cnt"]+=1; stats["currency"][cur]["ok"]+=1; stats["currency"][cur]["qty"]+=q
            if ts: stats["last_ok"]=ts
            day=time.strftime("%Y-%m-%d",time.localtime(ts or time.time())); stats["per_day"][day]["qty"]+=q; stats["per_day"][day]["ok"]+=1; continue
        m=_re.search(r"SEND FAIL\s+(\d+)\s*⭐.*?@([A-Za-z0-9_]{4,32}):\s*(.+?)(?:\s*\|\s*status=(\d+))?$",ln)
        if m:
            q=int(m.group(1)); cur=(last or {}).get("cur","ton"); stats["fail"]+=1; stats["qty_fail"]+=q; stats["currency"][cur]["fail"]+=1; reason=(m.group(3) or "ошибка").strip()[:120]; stats["fail_reasons"][reason]+=1
            if ts: stats["last_fail"]=ts
            day=time.strftime("%Y-%m-%d",time.localtime(ts or time.time())); stats["per_day"][day]["fail"]+=1; continue
        m=_re.search(r"SEND result:\s*ok=(True|False).*?currency=([a-z0-9_]+)?.*?order_status=([A-Za-z_\-]+|-)?",ln)
        if m and last:
            cur=m.group(2) or last.get("cur","ton"); st=(m.group(3) or "").upper()
            if m.group(1)=="True":
                stats["ok"]+=1; stats["qty_ok"]+=last["qty"]; stats["per_user"][last["user"]]["qty"]+=last["qty"]; stats["per_user"][last["user"]]["cnt"]+=1; stats["currency"][cur]["ok"]+=1; stats["currency"][cur]["qty"]+=last["qty"]
                if st in {"PENDING","BLOCKCHAIN_SENT"}: stats["pending"]+=1
                if ts: stats["last_ok"]=ts
                day=time.strftime("%Y-%m-%d",time.localtime(ts or time.time())); stats["per_day"][day]["qty"]+=last["qty"]; stats["per_day"][day]["ok"]+=1
            else:
                stats["fail"]+=1; stats["qty_fail"]+=last["qty"]; stats["currency"][cur]["fail"]+=1
                if ts: stats["last_fail"]=ts
                day=time.strftime("%Y-%m-%d",time.localtime(ts or time.time())); stats["per_day"][day]["fail"]+=1
        m=_re.search(r"REFUND\s+#?([A-Za-z0-9\-]+)\s*->\s*(OK|FAIL)",ln)
        if m:
            if m.group(2)=="OK": stats["refunds_ok"]+=1
            else: stats["refunds_fail"]+=1
    stats["per_user"]=dict(stats["per_user"]); stats["per_day"]=dict(stats["per_day"]); stats["fail_reasons"]=dict(stats["fail_reasons"]); stats["currency"]=dict(stats["currency"]); return stats
...
def _stats_text(chat_id: Any, range_key: str) -> str:
    now=time.time(); ranges={"24h":(now-86400,"за 24 часа"),"7d":(now-604800,"за 7 дней"),"30d":(now-2592000,"за 30 дней"),"all":(None,"за всё время")}
    since_ts,label=ranges.get(range_key,ranges["7d"]); s=_stats_collect(_read_log_tail(),since_ts); total=s["ok"]+s["fail"]; conv=(s["ok"]/total*100.0) if total else 0.0; avg=(s["qty_ok"]/s["ok"]) if s["ok"] else 0.0
    top=sorted(s["per_user"].items(),key=lambda kv:kv[1]["qty"],reverse=True)[:7]; top_lines=[f"{i}) @{u} — {int(v['qty'])}⭐ ({v['cnt']} заказ.)" for i,(u,v) in enumerate(top,1)]
    days=sorted(s["per_day"].items())[-10:]; day_lines=[f"{d}: {int(v.get('qty',0))}⭐ / ✅{v.get('ok',0)} ❌{v.get('fail',0)}" for d,v in days] if days else ["—"]
    fails=sorted(s["fail_reasons"].items(),key=lambda kv:kv[1],reverse=True)[:5]; fail_lines=[f"• {cnt}× — {_h(reason)}" for reason,cnt in fails] if fails else ["—"]
    cur_lines=[f"• {_h(_stars_currency_label(cur))}: ✅{v.get('ok',0)} / ❌{v.get('fail',0)} / {int(v.get('qty',0))}⭐" for cur,v in sorted(s["currency"].items())]
    cfg=_get_cfg(chat_id); currency=_normalize_stars_currency(cfg.get("stars_currency")); advice=[]
    if conv and conv<90: advice.append("низкая конверсия — посмотри последние причины ошибок")
    if any("jetton" in r.lower() or "комисс" in r.lower() or "low ton" in r.lower() for r in s["fail_reasons"]): advice.append("для USDT проверь USDT-jetton wallet и запас TON на комиссию")
    if not advice and total: advice.append("система работает стабильно")
...
def _star_act_all(bot, call):
    ...
    _set_cfg(
        ...
        lots_active=True,
        managed_lot_ids=managed_ids,
        last_auto_deact_reason=None,
        last_lot_toggle_report=f"star_act_all: {_lot_report_short(rep)}",
        last_lot_toggle_ts=int(time.time()),
    )
    try: bot.answer_callback_query(call.id, "Лоты включены.")
    except Exception: pass
...
def _star_deact_all(bot, call):
    ...
    _set_cfg(
        ...
        lots_active=False,
        managed_lot_ids=managed_ids,
        last_lot_toggle_report=f"star_deact_all: {_lot_report_short(rep)}",
        last_lot_toggle_ts=int(time.time()),
    )
    try: bot.answer_callback_query(call.id, "Лоты выключены.")
    except Exception: pass
    _open_stars(bot, call)
def _star_toggle(bot, call):
    ...
    ok = True
    if _CARDINAL_REF is not None:
        if enabled: ok = _activate_lot(_CARDINAL_REF, lot_id, trusted=True)
        else:       ok = _deactivate_lot(_CARDINAL_REF, lot_id, trusted=True)
    ...
    _set_cfg(
        ...
        managed_lot_ids=managed_ids,
        last_auto_deact_reason=(None if enabled and ok else cfg.get("last_auto_deact_reason")),
        last_lot_toggle_report=f"star_toggle lot={lot_id} enabled={enabled} ok={ok}",
        last_lot_toggle_ts=int(time.time()),
    )
    try:
...
def _autoadd_star_lots(cardinal: "Cardinal", chat_id: Any, category_id: int) -> dict:
    ...
    logger.info(
        ...
        f"found={len(lot_pairs)} added={added} updated={updated} unchanged={unchanged} "
        f"skipped={skipped} errors={errors} fallback={fallback_used} elapsed={elapsed:.2f}s"
    )
...
def _toggle_price_notifications(bot, call):
    ...
    try: bot.answer_callback_query(call.id, "Уведомления цен включены." if v else "Уведомления цен выключены.")
    except Exception: pass
...
def _toggle_auto_price_fragment(bot, call):
    ...
    try: bot.answer_callback_query(call.id, "Автоизменение цен включено." if v else "Автоизменение цен выключено.")
    except Exception: pass
    _open_pricing(bot, call)
def _toggle_autodump(bot, call):
    chat_id=call.message.chat.id; v=not _cfg_bool(_get_cfg(chat_id),"autodump_enabled",False); _set_cfg(chat_id,autodump_enabled=v)
    try: bot.answer_callback_query(call.id,"Автодемп включён." if v else "Автодемп выключен.")
    except Exception: pass
    _open_autodump(bot,call)
def _toggle_autodump_notifications(bot, call):
    chat_id=call.message.chat.id; v=not _cfg_bool(_get_cfg(chat_id),"autodump_notifications",True); _set_cfg(chat_id,autodump_notifications=v)
    try: bot.answer_callback_query(call.id,"Уведомления автодемпа включены." if v else "Уведомления автодемпа выключены.")
    except Exception: pass
    try: _open_autodump(bot,call)
    except Exception: _open_notifications(bot,call)
def _ask_autodump_interval(bot, call):
    chat_id=call.message.chat.id; cur=int(_get_cfg(chat_id).get("autodump_interval_sec",1800))//60; _fsm[chat_id]={"step":"set_autodump_interval"}
    try: bot.answer_callback_query(call.id)
    except Exception: pass
    ...
    st=_fsm.get(chat_id) or {}; st["prompt_msg_id"]=getattr(m,"message_id",None); _fsm[chat_id]=st
def _cb_autodump_run(cardinal:"Cardinal", call):
    bot=cardinal.telegram.bot; chat_id=call.message.chat.id
    try: bot.answer_callback_query(call.id,"Проверяю цены конкурентов…")
    except Exception: pass
    ok,msg=_apply_autodump(cardinal,chat_id,manual=True); bot.send_message(chat_id,msg,parse_mode="HTML")
    try: _open_autodump(bot,call)
    except Exception: pass
...
def _toggle_autosend_plus(bot, call):
    ...
    _open_mini_settings(bot, call)
def _ask_order_timer(bot, call, *, step: str, cfg_key: str, title: str, default_sec: int, min_min: int, max_min: int, example: int) -> None:
    chat_id = call.message.chat.id
    ...
    m = bot.send_message(
        chat_id,
        f"Введите время в минутах для «{title}» от {min_min} до {max_min}.\nСейчас: {cur_min} мин. Пример: {example}\n(или /cancel)",
        reply_markup=_kb_cancel_fsm()
    )
    st = _fsm.get(chat_id) or {}; st["prompt_msg_id"] = getattr(m, "message_id", None); _fsm[chat_id] = st
...
def _toggle_order_watch(bot, call):
    chat_id = call.message.chat.id; v = not _cfg_bool(_get_cfg(chat_id), "order_watch_enabled", True)
    _set_cfg(chat_id, order_watch_enabled=v)
    try: bot.answer_callback_query(call.id, "Проверка зависших заказов включена." if v else "Проверка зависших заказов выключена.")
    except Exception: pass
    _open_order_tools(bot, call)
def _toggle_review_reminder(bot, call):
    chat_id = call.message.chat.id; v = not _cfg_bool(_get_cfg(chat_id), "order_review_reminder_enabled", True)
    _set_cfg(chat_id, order_review_reminder_enabled=v)
    try: bot.answer_callback_query(call.id, "Напоминание об отзыве включено." if v else "Напоминание об отзыве выключено.")
    except Exception: pass
    _open_order_tools(bot, call)
def _cb_order_watch_run(cardinal: "Cardinal", call):
    bot = cardinal.telegram.bot; chat_id = call.message.chat.id
    try: bot.answer_callback_query(call.id, "Проверяю зависшие заказы…")
    except Exception: pass
    def _run():
        w, r, _ = _order_health_check(cardinal, chat_id, manual=True)
        try: bot.send_message(chat_id, f"✅ Проверка заказов завершена. Напоминаний по ожиданию: {w}; просьб об отзыве: {r}.")
        except Exception: pass
    if not _schedule_job(f"order-watch:{chat_id}", _run):
        try: bot.answer_callback_query(call.id, "Проверка уже выполняется.", show_alert=True)
        except Exception: pass
...
def _toggle_lots(bot, call):
    ...
    if _CARDINAL_REF is not None:
        if star_lots:
            ...
            if desired and not known_ids and not (rep.get("ok") or rep.get("skip")):
                bot.answer_callback_query(
                    call.id,
                    "Не нашёл лоты для включения. Запустите «Автодобавление лотов» в настройке лотов или добавьте LOT вручную.",
                    show_alert=True,
                )
                _open_settings(bot, call)
                return
    ...
    updates = {
        "lots_active": desired,
        "managed_lot_ids": known_ids,
        "last_lot_toggle_report": f"settings_toggle enabled={desired}: {_lot_report_short(rep)}",
        "last_lot_toggle_ts": int(time.time()),
    }
    if star_lots:
...
def _toggle_balance_lot_filter(bot, call):
    chat_id=call.message.chat.id; cfg=_get_cfg(chat_id); cfg=_set_cfg(chat_id,balance_lot_filter_enabled=not bool(cfg.get("balance_lot_filter_enabled",True)))
    try: bot.answer_callback_query(call.id,"Фильтр по балансу включён." if cfg["balance_lot_filter_enabled"] else "Фильтр по балансу выключен.")
    except Exception: pass
    _open_pricing(bot,call)
def _cb_balance_lot_filter_run(cardinal:"Cardinal", call):
    bot=cardinal.telegram.bot; chat_id=call.message.chat.id
    try: bot.answer_callback_query(call.id,"Проверяю лоты по балансу…")
    except Exception: pass
    ok,msg=_apply_balance_lot_filter(cardinal,chat_id,_get_cfg(chat_id),force=True); bot.send_message(chat_id,("✅ " if ok else "⚠️ ")+_h(msg),parse_mode="HTML")
    try: _open_pricing(bot,call)
    except Exception: pass
    ...
    if cfg.get("fragment_jwt"):
        ...
        if ver is not None or bal is not None or usdt is not None or raw is not None:
            _set_cfg(
                ...
                balance_usdt=(round(usdt, 6) if isinstance(usdt, (int, float)) else None),
                last_wallet_raw=raw
            )
    if _CARDINAL_REF is not None:
        cfg2=_get_cfg(chat_id); _maybe_auto_price_update(_CARDINAL_REF, chat_id); _maybe_autodump_update(_CARDINAL_REF, chat_id); _apply_balance_lot_filter(_CARDINAL_REF, chat_id, cfg2)
    _open_settings(bot, call)
    try: bot.answer_callback_query(call.id, "Обновлено.", show_alert=False)
    except Exception: pass
def _ask_set_min_balance(bot, call):
    chat_id = call.message.chat.id
    cfg = _get_cfg(chat_id); cur_code = _normalize_stars_currency(cfg.get("stars_currency"))
...
def _ask_set_jwt(bot, call):
    ...
    m = bot.send_message(
        chat_id,
        "Пришлите готовый JWT текстом или файлом (.txt / .json). "
...
def _is_jwt_like(s: str) -> bool:
    if not s or s.count(".") < 2: return False
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                if k in CAND_KEYS and isinstance(v, str):
                    ...
                    if _is_jwt_like(cand) or len(cand) > 16: return cand
...
def _handle_fsm(message: Message, cardinal: Cardinal):
    ...
    if state.get("step") == "autodump_floor_value":
        pmid=(_fsm.get(chat_id) or {}).get("prompt_msg_id"); lot_id=int(state.get("lot_id")); cur=state.get("currency") or "RUB"
        if text.lower() in ("/cancel","cancel","отмена"):
            _safe_delete(cardinal.telegram.bot, chat_id, pmid); _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None)); _fsm.pop(chat_id,None); return
        raw=text.replace(",",".").strip().lower()
        try:
            val=None if raw in ("0","-","нет","off","none") else float(raw)
            if val is not None and val<0: raise ValueError
        except Exception:
            _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None)); cardinal.telegram.bot.send_message(chat_id,"⚠️ Введите число ≥ 0, 0 или '-' для удаления порога."); return
        cfg=_get_cfg(chat_id); items=cfg.get("star_lots") or []; ok=False
        for it in items:
            if int(it.get("lot_id",0))==lot_id:
                if val is None: it.pop("autodump_min_price",None)
                else: it["autodump_min_price"]=round(float(val),2)
                ok=True; break
        _set_cfg(chat_id,star_lots=items); _safe_delete(cardinal.telegram.bot, chat_id, pmid); _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None)); _fsm.pop(chat_id,None)
        ...
        try: _open_stars(cardinal.telegram.bot, type("obj", (), {"message": type("m", (), {"chat": type("c", (), {"id": chat_id})(), "id": message.message_id})(), "id": ""}))
        except Exception: pass
        return
    ...
    if state.get("step") == "star_add_lotid":
        ...
        _set_cfg(chat_id, star_lots=items, lots_active=True, managed_lot_ids=_merge_lot_ids(_managed_lot_ids_from_cfg(cfg), [lot_id]), last_auto_deact_reason=None); _fsm.pop(chat_id, None)
        if _CARDINAL_REF is not None: _activate_lot(_CARDINAL_REF, lot_id, trusted=True)
    ...
    if state.get("step") in {"set_order_watch_interval", "set_order_wait_reminder", "set_review_reminder_time"}:
        ...
        try: _open_order_tools(cardinal.telegram.bot, type("obj", (), {"message": type("m", (), {"chat": type("c", (), {"id": chat_id})(), "id": getattr(message,"message_id",0)})(), "id": ""}))
        except Exception: pass
        return
    if state.get("step") == "set_autodump_interval":
        pmid=(_fsm.get(chat_id) or {}).get("prompt_msg_id")
        if text.lower() in ("/cancel", "cancel", "отмена"):
            _safe_delete(cardinal.telegram.bot, chat_id, pmid); _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None))
            _fsm.pop(chat_id, None); cardinal.telegram.bot.send_message(chat_id, "❌ Отменено."); return
        try:
            mins=int(float(text.replace(",",".").strip()))
            if mins<10 or mins>1440: raise ValueError
        except Exception:
            _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None))
            cardinal.telegram.bot.send_message(chat_id,"⚠️ Введите число минут от 10 до 1440. Пример: 30"); return
        _set_cfg(chat_id, autodump_interval_sec=mins*60); _safe_delete(cardinal.telegram.bot, chat_id, pmid); _safe_delete(cardinal.telegram.bot, chat_id, getattr(message,"message_id",None)); _fsm.pop(chat_id,None)
        cardinal.telegram.bot.send_message(chat_id,f"✅ Интервал автодемпа сохранён: {mins} мин.")
        try: _open_autodump(cardinal.telegram.bot, type("obj", (), {"message": type("m", (), {"chat": type("c", (), {"id": chat_id})(), "id": getattr(message,"message_id",0)})(), "id": ""}))
        except Exception: pass
        return
    ...
    if state.get("step") == "saves_import":
        ...
        if getattr(message, "document", None):
            try:
                if message.document.file_size and message.document.file_size > 5_000_000:
                    _safe_delete(cardinal.telegram.bot, chat_id, getattr(message, "message_id", None))
...
def _finalize_order(oid: str, chat_id: Any, *, ok: bool, reason: str = ""):
    oid = str(oid)
    if ok:
        _done_oids.add(oid); _order_record_update(chat_id, oid, status="sent", finalized_ts=int(time.time()))
    else:
        _blocked_oids.add(oid); _failed_orders[oid] = {"chat_id": chat_id, "reason": reason, "ts": time.time()}
...
def _set_order_qty(chat_id: Any, order_id: Optional[str], qty: Optional[int]) -> None:
    if not order_id or not qty or qty < 50: return
    try:
...
def _adopt_foreign_queue_for(chat_id: Any) -> bool:
    key = str(chat_id)
    if key in _pending_orders and _pending_orders[key]: return False
...
def _was_prompted(chat_id: Any, order_id: Optional[Any]) -> bool:
    if order_id is None: return any(x.get("prompted") for x in _q(chat_id))
    oid = str(order_id)
    if oid in _prompted_oids: return True
...
def _ensure_pending(chat_id: Any, order_id: Optional[str], qty: Optional[int]) -> Dict[str, Any]:
    if order_id and (str(order_id) in _done_oids or str(order_id) in _blocked_oids): return {
            "qty": int(qty or 50),
            "order_id": order_id,
            "chat_id": chat_id,
            "stage": "finalized",
...
def _prompt_current_order_if_needed(cardinal: "Cardinal", chat_id: Any) -> None:
    ...
    if not cur: return
    if cur.get("finalized") or not _allowed_stages(cur): return
    oid = cur.get("order_id")
    qty = int(cur.get("qty") or 50)
    if cur.get("prompted") or _was_prompted(chat_id, oid): return
    if not _should_prompt_once(chat_id, oid, qty): return
...
def _is_auto_reply(msg) -> bool:
    try:
        if getattr(msg, "is_autoreply", False): return True
        badge = (getattr(msg, "badge", None) or getattr(msg, "badge_text", None) or "")
        if isinstance(badge, str) and "автоответ" in badge.lower(): return True
        if getattr(msg, "by_bot", False) or getattr(msg, "by_vertex", False): return True
    except Exception: pass
    return False
...
def _queue_watchdog(cardinal: "Cardinal"):
    while True:
        ...
        try:
            for _ in range(5):
                if not _maybe_rotate_queue_head(cardinal, "__orders__"): break
        except Exception as e:
            logger.debug(f"queue_watchdog failed: {e}")
def new_order_handler(cardinal: Cardinal, event):
    chat_id = _event_chat_id(event)
    for _ in range(3):
        if not _maybe_rotate_queue_head(cardinal, chat_id if chat_id is not None else "__orders__"): break
    try:
        chat_id = _event_chat_id(event)
        cfg = _get_cfg_for_orders(chat_id if chat_id is not None else "__orders__")
        if not cfg.get("plugin_enabled", True): return
...
def _meta_guard() -> bool:
    ...
    if not _IMMUTABLE_OK: return False
...
def _do_confirm_send(cardinal: "Cardinal", chat_id):
    ...
    for _ in range(3):
        if not _maybe_rotate_queue_head(cardinal, chat_id): break
    if not pend:
    ...
    if _resp_indicates_delivery(resp):
        ...
        if oid: _finalize_order(oid, chat_id, ok=True)
        else: _pop_current(chat_id, keep_prompted=False)
        if _has_queue(chat_id): _notify_next_turn(cardinal, chat_id)
        return
...
def _allowed_stages(item: dict) -> bool:
    ...
    if stage == "await_paid": return not item.get("finalized")
    return (
...
def _pending_by_oid(chat_id: Any, oid: Optional[str]) -> Optional[dict]:
    ...
    for x in _q(chat_id):
        if str(x.get("order_id")) == str(oid) and _allowed_stages(x): return x
    return None
def _apply_username_for_item(cardinal: "Cardinal", chat_id: Any, item: dict, uname: str):
    ...
    if item.get("preconfirmed"):
        ...
        return
    if _auto_send_without_plus(chat_id):
        if FTS_GLOBAL_QUEUE and (_current(chat_id) is not item) and (_queue_mode(chat_id) != 2): return
    ...
    _safe_send(
        ...
        "Чтобы изменить — пришлите другой тег формата @username."
...
def new_message_handler(cardinal: Cardinal, event: NewMessageEvent):
    ...
    try:
        ...
        if author == "funpay" and (_is_gift_like_text(text) or _mentions_account_login(text)):
            ...
            return
        if _is_sending(chat_id) and (author != "funpay"): return
        cfg = _get_cfg_for_orders(chat_id)
        try:
        ...
        if author == "funpay" and _funpay_is_system_paid_message(text):
            ...
            if oid and (str(oid) in _done_oids or str(oid) in _blocked_oids): return
            _set_order_qty(chat_id, oid, qty)
            if qty is not None and qty < 50: return
            ...
            if uname and jwt and use_pre:
                ...
                if oid: _schedule_confirm_send(cardinal, chat_id, str(oid))
                else: _schedule_confirm_send(cardinal, chat_id)
                return
            ...
            return
        if not text: return
        if author == my_user and (_current(chat_id) or {}).get("stage") not in {"await_username", "await_confirm"}: return
        if author == "funpay":
            u = _extract_username_from_text(text)
            if not u or u.lower() == my_user.lstrip("@"): return
        ...
        m_plus = _re.match(r'^\s*(?:\+{1,2}|ok|да)\s*$', text, _re.I)
        ...
        if _auto_send_without_plus(chat_id):
            if FTS_GLOBAL_QUEUE and (pend is not _current(chat_id)): return
            _schedule_confirm_send(cardinal, chat_id)
            return
        _safe_send(
            cardinal, chat_id,
